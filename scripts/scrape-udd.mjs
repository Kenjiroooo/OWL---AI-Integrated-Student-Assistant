 ...
 * public/udd-context.txt with fresh content for the OWL AI assistant.
 *
 * Usage:
 *   npm run scrape
...
const OUTPUT_FILE = join(PROJECT_ROOT, 'public', 'udd-context.txt');
...
function generateTextFile(blocks, stats) {
...
`);
  ...
  const textContent = generateTextFile(contentBlocks, stats);
  writeFileSync(OUTPUT_FILE, textContent, 'utf-8');

  const fileSizeKB = (Buffer.byteLength(textContent, 'utf-8') / 1024).toFixed(1);
...
╚══════════════════════════════════════════════╝${C.reset}
  ...
  Output file:          ${C.cyan}public/udd-context.txt${C.reset}
