...
import { MessageSquare, Send, CheckCircle2, Sparkles, BrainCircuit } from 'lucide-react';
...
async function evaluateFeedback(
  category: string,
  content: string
): Promise<FeedbackEvaluation | null> {
  try {
    const res = await fetch('/api/evaluateFeedback', {
      method: 'POST',
...
const CATEGORIES = ['General', 'Facilities', 'Academic', 'Food Service'];
...
export default function FeedbackCenter() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Write the base feedback document immediately
      ...
      // Show success to the student right away
      setSubmitted(true);
      setContent('');

      // 2. Fire-and-forget: evaluate in the background and patch the document
      evaluateFeedback(category, content).then(async (evaluation) => {
        ...
        try {
          ...
          // Non-critical — feedback is already saved, evaluation just enriches it
          console.warn('Could not patch evaluation onto feedback document:', err);
        }
      });
    } catch (err) {
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center p-10 text-center">
        ...
        <motion.p
          ...
          className="text-xl text-slate-500 font-medium max-w-md mb-10 leading-relaxed"
        >
